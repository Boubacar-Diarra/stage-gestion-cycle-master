...
$donnee = $db->query('select count(*) as nb_message from coordinateur')->fetch();
...
{
    if($i != $nombreDePage)
        echo '<a class="pagination" title="Page: '.$i.'" href="http://localhost/gemProp/views/acceuil.php?pageaffiche=coordinateur&page='. $i .'"> '. $i . ' </a> , ';
    else
        echo '<a class="pagination" title="Page: '.$i.'" href="http://localhost/gemProp/views/acceuil.php?pageaffiche=coordinateur&page='. $i .'"> '. $i . ' </a> .';
}
echo "<hr/>";
...
$reponse = $db->query('select * from coordinateur limit ' . $premierMessageAAfficher .',' .$nombreDeMessageParPage);
echo '        <table>
            <caption>Coordinateurs du cycle master</caption>
            <tr>
                <th>Matricule</th>
                <th>Nom</th>
                <th>Prénom</th>
...
{       
        ...
        if($i%2 == 0)
        {
            ...
            echo '<td>
    <button>
        <a href="../views/ModifierCoordinateur.php?cne='.$donnee['cne'].'&page='.$page.'"><img src="../public/image/modify.png" alt="édition" title="éditer" class="buttonAction"></a>
    </button>
    <button>
        <a href="../controllers/SuppressionCoordinateur.php?cne='.$donnee['cne'].'"><img src="../public/image/delete.png" alt="supprimer" title="supprimer" class="buttonAction"></a>
    </button>
</td>';
            echo "</tr>";
        }
    ...
    <button>
        <a href="../controllers/SuppressionCoordinateur.php?cne='.$donnee['cne'].'" class="confirmSuppression"><img src="../public/image/delete.png" alt="supprimer" title="supprimer" class="buttonAction"></a>
    </button>
</td>';
            echo "</tr>";
        }
